...
1. LANGUAGE PREFERENCE (MANDATORY - FIRST STEP)
...
Introduce yourself:
When you start, introduce yourself as StoryBoom AI — your storytelling partner.
...
→ This gives the conversation a relaxed feel.
...
3. GATHER FIVE ESSENTIAL INTRO FIELDS (MANDATORY EARLY STEPS)
These five are:
Their name
The name of the company or team they represent (solution provider)
Their role / job title
Who the work was for — the client name or audience
The name of the project, product, or solution being discussed
...
4. SET EXPECTATIONS EARLY (MANDATORY)
...
5. MAIN INTERVIEW FLOW — OPEN-ENDED, HUMAN, DYNAMIC QUESTIONS
STORY QUESTION FLOW:
Client Overview (context about the client)
...
Optionally: "How big is their team or presence?" or "Do they have any particular values or goals that tied into this project?"
The Challenge
...
The Solution (use the project name from earlier)
...
6. CONTEXTUAL INTELLIGENCE & REACTIONS THROUGHOUT
...
7. HANDLE INTERRUPTIONS NATURALLY
If user interrupts, treat it like a natural interruption in a live conversation. Pause and wait for them to finish.
Resume conversation seamlessly from where left off — don't restart your last sentence unless they ask.
8. ENDING & WRAP-UP (MANDATORY SIX-STEP PROCESS)
Follow these steps with small pauses between them for a casual, friendly call ending:
Start Wrap-Up Naturally
...
→ Pause briefly; wait for user response.
Mention the Client Involvement
...
→ Pause briefly.
Explain What the Client Link Does
...
→ Pause.
Explain the Summary and What Happens Next
...
→ Let user absorb this.
Reassure About Control and Edits
...
→ Pause warmly.
End the Conversation Clearly and Kindly
...
If you picked up on any personal context at the start (like BBQ plans), reference it in a friendly way: "Enjoy that BBQ later!"
→ Wait a moment, then end session.
9. STYLE & TONE SUMMARY
...
async function endConversation(reason) {
  ...
  // For timeout, give grace period for final transcription events
  if (reason === '10-minute limit reached.') {
    // Wait for final transcription events to complete
    setTimeout(() => {
  ...
  setTimeout(async () => {
    ...
    console.log("transcriptLog length:", transcriptLog.length);
    console.log("transcriptLog contents:", transcriptLog);
    
    const formattedTranscript = transcriptLog
      .map(e => `${e.speaker.toUpperCase()}: ${e.text}`)
      .join("\n");
    
    console.log("formattedTranscript length:", formattedTranscript.length);
    console.log("formattedTranscript preview:", formattedTranscript.substring(0, 200));
...
function handleMessage(event) {
  ...
  switch (msg.type) {
    ...
    case "conversation.item.input_audio_transcription.completed":
      if (msg.transcript) {  // Removed !hasEnded check to allow final events
        transcriptLog.push({ speaker: "user", text: msg.transcript });
        const cleanedText = msg.transcript.toLowerCase().trim();
        userBuffer = "";

        if (isFarewell(cleanedText) && !hasEnded) {  // Only check hasEnded for farewell detection
    ...
    async function saveSummary() {
      try {
        ...
        if (result.status === 'success') {
          ...
          // Also save as final summary with edited names
          if (caseStudyId) {
            // Extract current names from the nameMap
            const solution_provider = nameMap['Solution Provider'] || '';
            const client_name = nameMap['Client'] || '';
            const project_name = nameMap['Project'] || '';
            
            try {
              const finalRes = await fetch('/api/save_final_summary', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                  case_study_id: caseStudyId,
                  final_summary: currentText,
